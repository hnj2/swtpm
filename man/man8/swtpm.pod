...
When providing the I<incoming> option parameter swtpm will defer the
locking of the NVRAM until the state blobs are received or until the
first TPM command is processed if no state blobs are received.
